...
export function DashboardPage() {
  ...
  if (!events.length) return <EmptyState icon={<CalendarCheck2 size={36} />} title="Sua agenda está vazia." description="Importe seu primeiro cronograma para descobrir sua próxima aula com zero cliques." action={<Link className="button button-primary button-md" to="/imports/new"><Import size={16} /> Importar CSV</Link>} />

  const todayEvents = events.filter((event) => event.eventDate === today)
  const next = nextScheduleEvent(events, now, timeZone) as EventWithRelations | null
...
function NextEvent({ event, now, timeZone, todayCount }: { event: EventWithRelations; now: Date; timeZone: string; todayCount: number }) {
  const state = getTemporalEventState(event, now, timeZone)
  const isToday = event.eventDate === todayIso(now, timeZone)
  const heading = state === 'in_progress' ? 'Aula em andamento' : state === 'soon' ? 'Começa em breve' : isToday ? `Você tem ${todayCount} ${todayCount === 1 ? 'aula' : 'aulas'} hoje` : 'Sua próxima aula'
  return <div className="next-event"><Badge tone={state === 'in_progress' ? 'positive' : state === 'pending_time' ? 'warning' : 'accent'}>{heading}</Badge><div className="next-class"><span className={`class-bar color-${event.classColor ?? 'teal'}`} /><div><strong>{event.classCode}</strong><span>{isToday ? 'Hoje' : formatLongDate(event.eventDate).replace(/ de 2026$/, '')}</span></div></div><h2>{event.title}</h2><div className="event-time"><Clock3 size={19} /><strong>{event.timeStatus === 'pending' ? 'Horário a confirmar' : `${event.startTime} → ${event.endTime}`}</strong></div>{state === 'in_progress' && <p>Termina às {event.endTime}</p>}</div>
}

function Kpi({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) { return <Card className="kpi"><span>{icon}</span><div><strong>{value}</strong><p>{label}</p></div></Card> }
function EventRow({ event, today }: { event: EventWithRelations; today: string }) { return <div className="event-row"><div className="date-tile"><strong>{new Date(`${event.eventDate}T00:00:00Z`).getUTCDate()}</strong><span>{new Intl.DateTimeFormat('pt-BR', { month: 'short', timeZone: 'UTC' }).format(new Date(`${event.eventDate}T00:00:00Z`)).replace('.', '')}</span></div><span className={`class-line color-${event.classColor ?? 'teal'}`} /><div className="event-row-copy"><div><Badge>{event.classCode}</Badge>{event.actionText && <Badge tone="warning">Ação</Badge>}</div><strong>{event.title}</strong><span>{event.eventDate === today ? 'Hoje' : formatLongDate(event.eventDate).split(',')[0]} · {event.startTime ?? 'A confirmar'}</span></div></div> }
function WeekStrip({ events, today }: { events: EventWithRelations[]; today: string }) { const start = weekStart(today); return <div className="week-strip">{Array.from({ length: 7 }, (_, index) => addDays(start, index)).map((date) => { const items = events.filter((event) => event.eventDate === date); return <div key={date} className={date === today ? 'today' : ''}><span>{new Intl.DateTimeFormat('pt-BR', { weekday: 'short', timeZone: 'UTC' }).format(new Date(`${date}T00:00:00Z`)).replace('.', '')}</span><strong>{Number(date.slice(-2))}</strong><div>{items.map((item) => <i key={item.id} className={`color-${item.classColor ?? 'teal'}`} title={item.classCode} />)}</div></div> })}</div> }
function greetingFor(now: Date, timeZone: string) { const hour = Number(new Intl.DateTimeFormat('pt-BR', { hour: '2-digit', hour12: false, timeZone }).format(now)); return hour < 12 ? 'Bom dia' : hour < 18 ? 'Boa tarde' : 'Boa noite' }
function relativeDate(value: string) { return new Intl.RelativeTimeFormat('pt-BR', { numeric: 'auto' }).format(Math.round((new Date(value).getTime() - Date.now()) / 86_400_000), 'day') }
function shortDate(value: string) { return new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: 'short', timeZone: 'UTC' }).format(new Date(`${value}T00:00:00Z`)) }
function formatHours(value: number) { return Number.isInteger(value) ? `${value}h` : `${String(value).replace('.', ',')}h` }
function weekStart(iso: string) { const date = new Date(`${iso}T00:00:00Z`); const day = date.getUTCDay() || 7; date.setUTCDate(date.getUTCDate() - day + 1); return date.toISOString().slice(0, 10) }
...
function filterCurrentWeek(events: EventWithRelations[], today: string) { const start = weekStart(today); const end = addDays(start, 6); return events.filter((event) => event.eventDate >= start && event.eventDate <= end) }
